...
- The first two mxCell elements are mandatory structural cells:
  - id="0" — root container, no parent attribute
  - id="1" with parent="0" — default layer
- All diagram elements must have parent="1" (or the id of a group/layer).
- IDs must be unique within the diagram. Use any string (e.g. "2", "node-1", "abc123").
- Vertices need vertex="1", edges need edge="1" — these are mutually exclusive.
- Edges should reference source and target via cell IDs. Edges without connections need explicit mxPoint sourcePoint/targetPoint.
- Always use uncompressed XML (no compressed="true"). AI should not generate compressed content.
- Style strings end with semicolon. Example: "rounded=1;whiteSpace=wrap;html=1;"
- Coordinates: origin (0,0) is top-left, x increases rightward, y increases downward.
...
| image | Image container | Requires image= |
...
Use with: shape=stencil(.) or shape=mxgraph...
...
- Valid XML: File is well-formed XML with proper escaping
- Root element: <mxfile> as root, contains at least one <diagram>
- Diagram IDs: Each diagram has a unique id attribute
- Structural cells: Root contains <mxCell id="0"/> and <mxCell id="1" parent="0"/>
- Unique IDs: All cell IDs are unique within the diagram
- Parent references: Every cell (except id="0") has a valid parent referencing an existing cell
- Type flags: Each content cell has either vertex="1" or edge="1" (not both)
- Edge references: Edge source and target reference existing vertex IDs
- Geometry: Vertices have mxGeometry with x, y, width, height; edges have relative="1"
- Style format: Style strings use key=value; format, valid keys and values
- Perimeter match: Non-rectangular shapes have matching perimeter in style
- HTML escaping: HTML in value attributes is properly XML-escaped
- Coordinate system: x increases right, y increases down, no negative dimensions
- Group hierarchy: Children of groups have coordinates relative to the group
